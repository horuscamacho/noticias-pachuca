  ...
  private setupInterceptors(): void {
    ...
    this.client.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        // Auto-refresh en 401 (token expirado) - EXCEPTO para endpoints de auth y publicaciones
        if (error.response?.status === 401 && originalRequest && !originalRequest._retry) {
          // ⚠️ CRÍTICO: NO reintentar endpoints de publicación para evitar duplicados
          const isPublishEndpoint = originalRequest.url?.includes('/publish') ||
                                    originalRequest.url?.includes('/schedule')

          if (isPublishEndpoint) {
            console.log('🚨 ApiClient interceptor - 401 on publish endpoint, NO retry to avoid duplicates')
            throw error
          }
