...
import { getCategories, getNoticiasByTag } from '../features/public-content';
...
export const Route = createFileRoute('/tag/$slug')({
  ...
  loader: async ({ params, context }) => {
    ...
    // Fetch noticias del tag
    const noticiasResponse = await getNoticiasByTag({
      data: {
        slug,
        page,
        limit: 20,
    ...
    // Extraer nombre del tag del slug
  ...
  head: ({ loaderData }) => {
    ...
    const breadcrumbSchema = generateBreadcrumbSchema([
      ...
      { name: `Tag: ${tag.name}`, url: `https://noticiaspachuca.com/tag/${tag.slug}` }
    ]);

    return {
      meta: [
        { title: `Tag: ${tag.name} - Noticias Pachuca` },
        { name: 'description', content: `Todas las noticias etiquetadas con ${tag.name}` },
        { property: 'og:title', content: `Tag: ${tag.name} - Noticias Pachuca` },
        { property: 'og:description', content: `Todas las noticias etiquetadas con ${tag.name}` },
        { property: 'og:type', content: 'website' },
        { property: 'og:url', content: `https://noticiaspachuca.com/tag/${tag.slug}` },
        { name: 'robots', content: 'index, follow, max-image-preview:large, max-snippet:-1' },
      ],
      links: [
        { rel: 'canonical', href: `https://noticiaspachuca.com/tag/${tag.slug}` }
      ],
      scripts: [
        {
...
function TagPage() {
  const { noticias, pagination, tag, categories } = Route.useLoaderData();
      ...
      <div className="bg-white border-b-4 border-black py-8 md:py-12">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="mb-4">
            ...
            <span className="text-sm font-bold uppercase tracking-wider text-[#FFB22C]">
              TAG: {tag.name}
            </span>
          </div>

          {/* Tag Title with Color Bar */}
          <div className="relative">
            <div
              className="absolute left-0 top-0 w-2 h-full bg-[#FFB22C]"
            ></div>
            <div className="pl-6">
              ...
              <p className="text-lg md:text-xl text-black max-w-3xl leading-relaxed">
                Todas las noticias etiquetadas con {tag.name}
              </p>
              <div className="mt-4 inline-block bg-black text-white px-4 py-2 font-mono font-bold text-sm uppercase border-2 border-black">
                {pagination.total} {pagination.total === 1 ? 'NOTICIA' : 'NOTICIAS'} CON ESTE TAG
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex justify-center items-center py-20">
            <div className="bg-white border-4 border-black p-12 text-center shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
              ...
              <p className="text-black text-lg">
                Este tag aún no tiene contenido publicado.
              </p>
              <Link
                to="/"
